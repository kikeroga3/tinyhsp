// For macOS:
// $ clang++ cutehspx.cpp -o cutehspx -std=c++11 -lglfw -framework OpenGL
//
// For MinGW:
// $ g++ -static cutehspx_beep.cpp -o cutehspx -std=gnu++11 -lglfw3dll -lopengl32 -mwindows
// $ g++ -static cutehspx.cpp -o cutehspx_c -std=gnu++11 -lglfw3dll -lopengl32
// or -std=gnu++11
// or -std=c++0x
// or -std=gnu++0x
// or -std=c++11
//
// For Linux:
// $ g++ cutehspx.cpp -o cutehspx -lglfw3 -lX11 -lXrandr -lXinerama -lXi -lXxf86vm -lXcursor -lGL -lpthread -ldl
//
...
void
command_poke(execute_environment_t* NHSP_UNUA(e), execute_status_t* s, int arg_num)
{
    // 引数の数をチェックする
    if (arg_num != 3) {
	...
	const auto i = value_calc_int(*p2);
	// ３つめの引数を取得する
	const auto p3 = stack_peek(s->stack_, arg_start + 2);
	const auto num = value_calc_int(*p3);

	// 指定された変数に代入
	void* data_ptr = v->variable_->data_;
	reinterpret_cast<char*>(data_ptr)[i] = num;
	stack_pop(s->stack_, arg_num);
}

void
command_beep(execute_environment_t* NHSP_UNUA(e), execute_status_t* s, int arg_num)
{
    // 引数の数をチェックする
    if (arg_num != 2) {
	...
	const auto freq = value_calc_int(*p1);
	const auto p2 = stack_peek(s->stack_, arg_start + 1);
	const auto msec = value_calc_int(*p2);
...
#else	// Linux or macOS
	FILE *tty;
	tty = fopen("/dev/console", "w");
	if (tty != NULL) {
		fprintf(tty, "\033[10;%d]\033[11;%d]\a", freq, msec);
		fflush(tty);
		usleep(msec * 1000);
		fprintf(tty, "\033[10;750]\033[11;100]");
		fclose(tty);
	}
#endif
...
// 関数実体
void
function_peek(execute_environment_t* NHSP_UNUA(e), execute_status_t* s, int arg_num)
{
	if (arg_num != 2) {
