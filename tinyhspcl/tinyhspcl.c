/*=============================================================================
// For macOS:
// $ clang++ tinyhspcl.c -o tinyhspcl -std=c++11
//
// For MinGW:
...
// 実行環境ユーティリティ
list_node_t* search_label( execute_environment_t* e, const char* name )
{
...
void command_mes( execute_environment_t* NHSP_UNUA(e), execute_status_t* s, int arg_num )
{
	...
	{
		raise_error( "mes: Argument should specify a string type." );
	}
...
// メモリ
void* xmalloc( size_t size, const char* block_name )
{
...
restart:
	...
	switch( p[0] )
	{
		...
		case ',':	++p; res->tag_ = TOKEN_COMMA; break;
			...
			// 演算子
		case '|':	++p; res->tag_ = TOKEN_OP_BOR; break;
		case '&':	++p; res->tag_ = TOKEN_OP_BAND; break;
		...
		case '<':
			...
			break;
		case '+':	++p; res->tag_ = TOKEN_OP_ADD; break;
		case '-':	++p; res->tag_ = TOKEN_OP_SUB; break;
		case '*':	++p; res->tag_ = TOKEN_OP_MUL; break;
		case '/':	++p; res->tag_ = TOKEN_OP_DIV; break;
		case '\\':	++p; res->tag_ = TOKEN_OP_MOD; break;

			// 代入
		case '=':
			++p;
			if ( p[0] == '=' )
			{ ++p; res->tag_ = TOKEN_OP_EQ; }
			else
...
int variable_calc_int( const variable_t& r, int idx )
{
	...
	switch( r.type_ )
	{
		case VALUE_INT:		return *reinterpret_cast<int*>( data_ptr );
		case VALUE_DOUBLE:	return static_cast<int>(*reinterpret_cast<double*>( data_ptr ));
...
double variable_calc_double( const variable_t& r, int idx )
{
	...
	switch( r.type_ )
	{
		case VALUE_INT:		return static_cast<double>(*reinterpret_cast<int*>( data_ptr ));
		case VALUE_DOUBLE:	return *reinterpret_cast<double*>( data_ptr );
...
char* variable_calc_string( const variable_t& r, int idx )
{
	...
	switch( r.type_ )
	{
		case VALUE_INT:		return create_string_from( *reinterpret_cast<int*>( data_ptr ) );
		case VALUE_DOUBLE:	return create_string_from( *reinterpret_cast<double*>( data_ptr ) );
...
value_t* create_value( const value_t& v )
{
	...
	switch( v.type_ )
	{
		...
		default: raise_error( "Copying an empty value and failed.@@ ptr=%p", &v );
	}
	return res;
}
...
void evaluate( execute_environment_t* e, execute_status_t* s, ast_node_t* n )
{
	...
	switch( n->tag_ )
	{
		...
		case NODE_MOD:
		{
			...
			switch( n->tag_ )
			{
				case NODE_BOR:		value_bor( l, *r ); break;
				case NODE_BAND:		value_band( l, *r ); break;
				case NODE_EQ:		value_eq( l, *r ); break;
				case NODE_NEQ:		value_neq( l, *r ); break;
				case NODE_GT:		value_gt( l, *r ); break;
				case NODE_GTOE:		value_gtoe( l, *r ); break;
				case NODE_LT:		value_lt( l, *r ); break;
				case NODE_LTOE:		value_ltoe( l, *r ); break;
				case NODE_ADD:		value_add( l, *r ); break;
				case NODE_SUB:		value_sub( l, *r ); break;
				case NODE_MUL:		value_mul( l, *r ); break;
				case NODE_DIV:		value_div( l, *r ); break;
				case NODE_MOD:		value_mod( l, *r ); break;
				default: assert( false ); break;
			}
			stack_pop( s->stack_ );
			break;
		...
		case NODE_UNARY_MINUS:
		{
			...
			switch( v->type_ )
			{
				...
				case VALUE_STRING:	raise_error( "no minus value in the string.[%s]", v->svalue_ ); break;
				default: assert( false ); break;
			}
			break;
		}
		case NODE_PRIMITIVE_VALUE:
		{
			...
			{
				case TOKEN_INTEGER:	stack_push( s->stack_, create_value( atoi( n->token_->content_ ) ) ); break;
				case TOKEN_REAL:	stack_push( s->stack_, create_value( atof( n->token_->content_ ) ) ); break;
				case TOKEN_STRING:	stack_push( s->stack_, create_value( n->token_->content_ ) ); break;
				default: assert( false ); break;
			}
			break;
		}
		...
		case NODE_CONTINUE:
		{
			if ( s->current_loop_frame_ <= 0 )
			{
				raise_error( "loop,continue: is nothing in repeat-loop." );
			}
		...
		case NODE_BREAK:
		{
			if ( s->current_loop_frame_ <= 0 )
			{
				raise_error( "break: is nothing in repeat-loop." );
			}
