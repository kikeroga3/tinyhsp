...
CuteHSPは Windows, macOS, Linux で同じスクリプトが実行できるクロスプラットフォームな処理系を目指した小さくてキュートなプログラム言語です。
...
###実行方法
スクリプトファイル(例：start.hs)を cutehsp.exe にドラッグ＆ドロップするか、
拡張子「.hs」を cutehsp.exe に関連付けしてスクリプトファイルをダブルクリックします。
